...
class Project {
  ...
  activate() {
    ...
    let y =
      window.innerHeight / 2 - (top + height / 2) - window.innerHeight * 0.1;
    if (this.active) {
...
function animate() {
  if (animating) {
    mouseCoordinates.x = lerp(
      mouseCoordinates.x,
      mouseCoordinates.targetX,
      0.075
    );
    mouseCoordinates.y = lerp(
      mouseCoordinates.y,
      mouseCoordinates.targetY,
      0.075
    );
